...
// بيانات وهمية للاستخدام عند عدم توفر بيانات من الـ API
const DEMO_USERS = [
  {
    id: '1',
    firstName: 'Sarah',
    ...
    try {
      ...
      if (response && (response.data || Array.isArray(response))) {
        ...
        if (usersData.length > 0) {
          ...
          // استخدام البيانات الوهمية إذا لم توجد بيانات حقيقية
          setUsers(DEMO_USERS);
          setPagination(prev => ({
            ...prev,
      ...
      // استخدام البيانات الوهمية عند الخطأ
      setUsers(DEMO_USERS);
      setPagination(prev => ({
        ...prev,
        totalPages: Math.ceil(DEMO_USERS.length / prev.pageSize),
  ...
  const fetchStatistics = useCallback(async () => {
    try {
      ...
      if (response) {
        ...
        // إحصائيات افتراضية من البيانات الوهمية
        setStats({
